...
퍼블 코드는 UI/디자인/CSS/className/HTML 요소/화면 구성의 절대 기준으로 보고, 내부 구조와 로직은 반드시 Notice 구현을 기준으로 다시 작성합니다.
...
- 퍼블된 코드/이미지: UI 모양, 화면 구성, 디자인, CSS, className, HTML 요소의 절대 기준
...
- 퍼블에 없는 UI 변경이 필요해 보이면 먼저 사용자에게 확인한다.
