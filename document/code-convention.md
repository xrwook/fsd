...
> **추천고함 스킬 파일**을 링크에서 상세 내용을 확인하세요.
...
모든 `import` 문에서는 `@` alias를 사용하여 절대 경로를 지정합니다.
...
```typescript
type TUserStatus = 'active' | 'inactive' | 'suspended';
...
interface Props {
  user: IUser;
  onStatusChange: (status: TUserStatus) => void;
}
```
...
| 종류 | 형식 | 예시 |
|---|---|---|
| 요청 파라미터 | `T...Params` | `TGetPostByIdParams` |
| 응답 데이터 | `T...Response` | `TGetPostByIdResponse` |

### API 경계에서의 Zod 검증

API 응답 데이터는 반드시 Zod로 검증합니다.  
`any` 타입 또는 단순 타입 캐스팅은 금지합니다.
...
const { data } = useQuery({
  queryKey: ['user'],
  queryFn: () => axios.get('/api/user').then((r) => r.data), // data 타입이 unknown 또는 any
});
```
...
const { data } = useQuery({
  queryKey: ['user'],
  queryFn: async () => userSchema.parse((await axios.get('/api/user')).data),
});
```
