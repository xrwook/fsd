...
3. [레이어별 슬라이스 구조](#레이어별-슬라이스-구조)
4. [Import 규칙](#import-규칙)
5. [세그먼트 구조](#세그먼트-구조)
6. [shared/ui 컴포넌트 파일 구조](#sharedui-컴포넌트-파일-구조)
7. [Public API 규칙](#public-api-규칙)
...
| 레이어      | ❌ 위반                                                  | 이유                                                                                               |
| ----------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| `shared/`   | 도메인 타입 import, 비즈니스 로직                        | shared는 도메인에 무관해야 모든 레이어에서 재사용 가능                                             |
| `entities/` | POST/PUT/DELETE API, mutation 상태, 유저 액션 핸들러     | entities는 읽기 전용 도메인 데이터 모델링 — 변경 의도(mutation)는 features에                       |
| `features/` | 도메인 타입 정의(+ entities), 도메인 무관 유틸(+ shared) | features는 도메인 + UI 로직 조합 — 도메인 자체는 정의하지 않음                                     |
...
| 케이스                                                                        | 위치      | 이유                                           |
| ----------------------------------------------------------------------------- | --------- | ---------------------------------------------- |
| 앱 전반에 걸친 Provider (라우터, 앱 전역 상태 등)                             | `app/`    | 앱 전역점에서 한 번만 마운트되는 전역 관심사   |
| `shared` 레이어와 관련된 Provider (공통 UI 컨텍스트, shared 유틸 Provider 등) | `shared/` | 제공하는 값의 출처와 소비자가 모두 shared 범위 |
...
// shared UI Provider를 app에 위치시키는 것
...
엔티티는 **여러 feature/page에서 공유되는 비즈니스 객체**입니다.

#### 슬라이스 기준

| 위치                 | 기준               | 예시                               |
| -------------------- | ------------------ | ---------------------------------- |
| `entities/{domain}/` | 특정 도메인 전용   | `entities/cpos/`, `entities/emsp/` |
| `entities/{entity}/` | 여러 도메인이 공유 | `entities/user/`                   |

엔티티를 올리는 기준: **두 곳 이상의 feature/page에서 동일 API/모델을 공유할 때**. 한 곳에서만 쓰이면 해당 page/feature 슬라이스 내부에 유지합니다.

#### 내부 구조

도메인 내부는 **세그먼트 기준 flat** 구조를 사용합니다.
...
│   ├── cpo.ts
│   ├── membershipCard.ts
│   └── station.ts       ← 엔티티별 파일이 api 안에 flat
├── model/
│   ├── cpo.ts
│   ├── membershipCard.ts
│   └── station.ts
...
feature는 **사용자 행동/인터랙션 단위**입니다.

#### 슬라이스 기준

도메인 그룹핑 없이 **기능명이 바로 슬라이스**입니다.
...
이름 충돌 발생 시 도메인 그룹핑 대신 **기능명을 더 구체적으로 명명**합니다.
...
같은 레이어 내 다른 슬라이스의 코드가 필요한 경우, 직접 import는 금지됩니다. 대신 **공개할 슬라이스에 `@x` 폴더**를 만들어 허용할 인터페이스만 노출합니다.
...
export { ChipFilter } from "./ChipFilter"; // ✅ _ 로 시작하는 것은 export하지 않음
```
...
`pages/` 하위는 화면 URL 기준으로 구성하되, 페이지가 많아져 뎁스가 길어지는 경우 **도메인 폴더 하위에 서브 그룹 폴더**를 둘 수 있습니다.

단, 서브 그룹 폴더는 단순 그룹핑 용도로만 사용하며, 별도의 `index.ts` export 파일을 두지 않습니다.
...
위 구조에서 `membership/`은 단순 그룹핑 폴더입니다.  
실제 페이지 슬라이스는 `membership-card-list/`, `membership-card-create/`, `membership-card-detail/`, `membership-card-edit/`입니다.
...
`membership/`은 단순 그룹핑 폴더이므로, 별도의 Public API 역할을 하는 `index.ts`를 두지 않습니다.

#### 도메인 폴더명 규칙

도메인별 폴더를 만들 때는 **소문자 케밥 케이스(kebab-case)** 를 사용합니다.
